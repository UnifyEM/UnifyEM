...
const serviceFile = "uem-server.service"
...
func (i *Install) installService() error {
	...
	// Set the target path
	targetPath := binaryPath + string(os.PathSeparator) + "uem-server"
...
func (i *Install) uninstallService(removeData bool) error {
	...
	// Remove the binary
	err = os.Remove(binaryPath + string(os.PathSeparator) + "uem-server")
	if err != nil {
...
func (i *Install) createService() error {
	...
	cmd := exec.Command("systemctl", "enable", "uem-server")
	err = cmd.Run()
	if err != nil {
...
func (i *Install) stopService() error {
	cmd := exec.Command("systemctl", "stop", "uem-server")
	err := cmd.Run()
	if err != nil {
...
func (i *Install) startService() error {
	cmd := exec.Command("systemctl", "start", "uem-server")
	err := cmd.Run()
	if err != nil {
